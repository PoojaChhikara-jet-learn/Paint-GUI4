...
class Paint:
    ...
    def __init__(self):
        ...
        # Tool buttons
        Button(self.screen, text="Pen", width=10, command=self.usePen).grid(
            row=1, column=1, padx=20
        )
        Button(self.screen, text="Brush", width=10, command=self.useBrush).grid(
            row=1, column=2, padx=20
        )
        Button(self.screen, text="Eraser", width=10, command=self.useEraser).grid(
            row=1, column=3, padx=20
        )
        Button(self.screen, text="Color", width=10, command=self.chooseColor).grid(
        ...
        Button(
            ...
            text="Canvas Color",
            width=12,
            command=self.chooseCanvasColor,
        ).grid(row=1, column=5, padx=20)
        Button(self.screen, text="Clear", width=10, command=self.clearCanvas).grid(
            row=1, column=6, padx=20
        )
        ...
        # Drawing canvas
        self.canvas = Canvas(self.screen, bg="white", width=w, height=h - 50)
        self.canvas.grid(row=2, column=0, columnspan=11, sticky="nsew")

        # Optional: make canvas expand when window is resized
    ...
    def saveImage(self):
        ...
        screenshot = pyautogui.screenshot(region=(x, y, cw, ch))
        screenshot.save(filePath)
    ...
    def paint(self, event):
        self.lineWidth = self.scaler.get()
        # Use current canvas bg color when erasing
        paintColor = self.canvas["bg"] if self.eraserUse else self.color

        # IMPORTANT FIX: check against None, not truthiness
